...
describe AppContentSetImporter do
  ...
  subject(:app_content_set) { AppContentSetImporter.new(content) }
  ...
  describe '.load!' do
    context 'with valid data' do
      it 'returns the app content set as an object' do
        expect(AppContentSetImporter.load!(valid_content_path)).to include(short_title: 'Goose Nuzzles')
      end
    end
  end
end
